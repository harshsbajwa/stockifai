...
class DataCollectionService(
    ...
    private val stockList: List<String> by lazy {
        stockSymbols.split(",").map { it.trim() }
    }
    ...
    fun collectStockData() {
        ...
        val yahooUrl = "https://query1.finance.yahoo.com/v7/finance/quote?symbols=$symbolsQuery"
            ...
            .doOnSuccess { response ->
                response.quoteResponse.result.forEach { quote ->
                    try {
                        ...
                        logger.debug("Sent stock data for {}: ${quote.regularMarketPrice}", quote.symbol)
                    } catch (e: Exception) {
                        logger.error("Error processing stock data for {}: {}", quote.symbol, e.message)
                    }
                }
                logger.info("Successfully collected data for {} stocks", response.quoteResponse.result.size)
            }
            .doOnError { error ->
    ...
    fun collectMarketVolatility() {
        ...
        val vixUrl = "https://query1.finance.yahoo.com/v7/finance/quote?symbols=^VIX"
    ...
    fun collectEconomicIndicators() {
        ...
        val fredUrl = "https://api.stlouisfed.org/fred/series/observations?series_id=FEDFUNDS&api_key=$fredApiKey&file_type=json&limit=1&sort_order=desc"
    ...
    fun collectAlphaVantageData() {
        ...
        val symbol = "SPY"
        val alphaUrl = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=$symbol&interval=5min&apikey=$alphaVantageApiKey"
        ...
        webClient.get()
            ...
            .doOnSuccess { response ->
                try {
                    logger.info("Received Alpha Vantage data for {}: {}", symbol, response.take(100) + "...")
                    
                    // Send raw data to processing
                    val message = StockMessage("intraday_data", mapOf("symbol" to symbol, "data" to response))
                    val messageJson = objectMapper.writeValueAsString(message)
