...
class NewsService(
    ...
    fun getRecentNews(hours: Long = 24, limit: Int = 50): List<NewsItem> {
        return try {
            ...
            return news.filter { Instant.ofEpochMilli(it.primaryKey.timestamp).isAfter(cutoffTime) }
                .take(limit)
    ...
    fun getNewsBySentiment(sentiment: String, hours: Long = 24, limit: Int = 20): List<NewsItem> {
        return getRecentNews(hours, limit * 3)
            .filter { it.sentiment.equals(sentiment, ignoreCase = true) }
            .take(limit)
    }

    fun getNewsForSymbol(symbol: String, hours: Long = 24, limit: Int = 20): List<NewsItem> {
        return getRecentNews(hours, limit * 5)
