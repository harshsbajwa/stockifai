...
class InfluxDBService(
    ...
    companion object {
        fun createClient(influxProperties: InfluxProperties): InfluxDBClientKotlin {
            ...
            val influxToken = System.getenv("INFLUXDB_TOKEN") ?: "YOUR_INFLUXDB_CLOUD_TOKEN_FALLBACK"
            if (influxToken == "YOUR_INFLUXDB_CLOUD_TOKEN_FALLBACK") {
    ...
    private fun getStockTimeSeriesBucket(): String {
        return System.getenv("INFLUXDB_STOCK_TIMESERIES_BUCKET") ?: influxProperties.bucket
    }

    private fun getEconomicTimeSeriesBucket(): String {
        return System.getenv("INFLUXDB_ECONOMIC_TIMESERIES_BUCKET") ?: influxProperties.bucket
    }
    ...
    fun getStockTimeSeries(
    ...
    ): StockMetricsResponse? = runBlocking {
        ...
        val calculatedMetricsQuery = """
            from(bucket: "$bucket")
              ...
              |> sort(columns: ["_time"])
        """.trimIndent()
        logger.debug("Executing InfluxDB Flux query for Calculated Metrics [{}]: {}", symbol, calculatedMetricsQuery)
        ...
        try {
            ...
            val metricPoints = mergedMetrics.entries.map { entry ->
                val time = entry.key
            ...
            if (metricPoints.isEmpty()) {
                logger.warn("No combined stock time-series data found for symbol {} in the last {} hours.", symbol, hours)
                return@runBlocking StockMetricsResponse(symbol, emptyList(), TimeRange(startTime, endTime), aggregationWindow)
            }
    ...
    fun getMarketVolatility(hours: Long = 24): List<MetricPoint> = runBlocking {
        val endTime = Instant.now()
        val startTime = endTime.minus(hours, ChronoUnit.HOURS)
        val bucket = getStockTimeSeriesBucket()
        val org = influxProperties.org
        ...
        try {
            ...
            val volatilityPoints = records.mapNotNull { record ->
                MetricPoint(
    ...
    fun getTopPerformingSymbols(limit: Int = 10): List<Pair<String, Double>> = runBlocking {
        val endTime = Instant.now()
        ...
        try {
            ...
            val performers = records.mapNotNull { record ->
    ...
    fun getEconomicTimeSeries(
        seriesId: String,
        days: Long = 30
    ): List<EconomicDataPoint>? = runBlocking {
        val endTime = Instant.now()
        ...
        val fluxQuery = """
            ...
            |> filter(fn: (r) => r.series_id == "$seriesId")
            |> filter(fn: (r) => r._field == "value")
            |> sort(columns: ["_time"])
            |> yield(name: "economic_data")
        """.trimIndent()

        logger.debug("Executing InfluxDB Flux for economic series [{}]: {}", seriesId, fluxQuery)
        ...
        try {
            ...
            val dataPoints = records.mapNotNull { record ->
                record.time?.let { time ->
                    (record.value as? Double)?.let { value ->
                        EconomicDataPoint(seriesId, value, time)
                    }
                }
            }

            if (dataPoints.isEmpty()) {
                logger.warn("No time-series data found for economic series_id {} in the last {} days.", seriesId, days)
            }

            dataPoints
        } catch (e: Exception) {
            logger.error("Error querying InfluxDB for economic series $seriesId: ${e.message}", e)
            null
        }
    }
}
