...
class InfluxDBServiceTest {
    ...
    private fun createMockFluxRecord(time: Instant, values: Map<String, Any?>): FluxRecord {
    ...
    private fun <T> createSingletonChannel(item: T): Channel<T> = Channel<T>(1).apply {
        trySend(item)
        close()
    }
    private fun <T> createListChannel(items: List<T>): Channel<T> = Channel<T>(items.size).apply {
        items.forEach { trySend(it) }
        close()
    }

    @Test
    fun `getStockTimeSeries should return merged data when both ohlcv and metrics present`() = runBlocking {
        val symbol = "AAPL"
        val now = Instant.now()

        val ohlcvRecord = createMockFluxRecord(now, mapOf("open" to 150.0, "high" to 152.0, "low" to 149.0, "close" to 151.0, "volume" to 1_000_000.0))
    ...
    @Test
    fun `getStockTimeSeries should return only ohlcv when metrics absent`() = runBlocking {
        val symbol = "MSFT"
        val now = Instant.now()
    ...
    @Test
    fun `getStockTimeSeries should return only metrics when ohlcv absent`() = runBlocking {
        val symbol = "GOOG"
        val now = Instant.now()
    ...
    @Test
    fun `getStockTimeSeries should handle empty results gracefully`() = runBlocking {
        val symbol = "EMPTY"
    ...
    @Test
    fun `getStockTimeSeries should return null on InfluxException`() = runBlocking {
        whenever(mockQueryKotlinApi.query(any<String>(), eq(testOrg)))
    ...
    @Test
    fun `getMarketVolatility should return aggregated volatility data`() = runBlocking {
        val now = Instant.now()
    ...
    @Test
    fun `getMarketVolatility should return empty list on InfluxException`() = runBlocking {
        whenever(mockQueryKotlinApi.query(any<String>(), eq(testOrg)))
            .thenThrow(InfluxException("Connection failed"))
    ...
    @Test
    fun `getTopPerformingSymbols should return sorted symbols`() = runBlocking {
        val now = Instant.now()
    ...
    @Test
    fun `getTopPerformingSymbols should return empty list on InfluxException`() = runBlocking {
         whenever(mockQueryKotlinApi.query(any<String>(), eq(testOrg)))
            .thenThrow(InfluxException("Connection failed"))
    ...
    @Test
    fun `getEconomicTimeSeries should return data points`() = runBlocking {
        val seriesId = "DGS10"
        val now = Instant.now()
        val recordTime = now.minus(1, ChronoUnit.DAYS)
        val record = createMockFluxRecord(recordTime, mapOf("_value" to 1.5, "series_id" to seriesId))
        ...
        val result = influxDBService.getEconomicTimeSeries(seriesId, 30)
    ...
    @Test
    fun `getEconomicTimeSeries should return null on InfluxException`() = runBlocking {
        whenever(mockQueryKotlinApi.query(any<String>(), eq(testOrg)))
            .thenThrow(InfluxException("Connection failed"))
    ...
    @Test
    fun `getEconomicTimeSeries should return empty list if no data points found`() = runBlocking {
        val seriesId = "NODATA"
        val emptyChannel = createEmptyChannel<FluxRecord>()
        whenever(mockQueryKotlinApi.query(any<String>(), eq(testOrg))).thenReturn(emptyChannel)
